...
- **Added self-awareness dashboard**: automated generation of `SELF_AWARENESS.md` that displays current operational state (version, budget, model usage, invariants, evolution status).
- Added `scripts/generate_self_awareness.py` to collect runtime state from Drive state file, git, and events logs.
- Added `make self-awareness` target for convenient regeneration.
- Created `SELF_AWARENESS.md` template with sections for identity, financial health, LLM statistics, evolution progress, invariants, and recent activity.
- Dashboard is auto-generated and reflects actual runtime state, satisfying continuity (P1) and self-creation (P2).

### v6.3.0 -- Stagnation Guard

- Added stagnation detection for evolution tasks to prevent infinite loops without commits.
- Improved health invariant reporting.